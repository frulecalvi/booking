...
//        var_dump($booking->tickets);
    ...
    public function test_pending_bookings_older_than_15_minutes_are_marked_as_expired_by_job_if_product_demands_payment()
    {
        $bookings = [
            Booking::factory()
                ->for($this->tours[0], 'bookingable')
                ->for($this->tours[0]->events()->first())
                ->create(['state' => BookingPending::$name]),
            Booking::factory()
                ->for($this->tours[1], 'bookingable')
                ->for($this->tours[1]->events()->first())
                ->create(['state' => BookingPending::$name, 'created_at' => now()->subMinutes(18)]),
            Booking::factory()
            ...
            Booking::factory()
                ...
                ->create(['state' => BookingPending::$name, 'created_at' => now()->subMinutes(15)]),
            Booking::factory()
                ->for($this->tours[4], 'bookingable')
                ->for($this->tours[4]->events()->first())
                ->create(['state' => BookingPaid::$name, 'created_at' => now()->subMinutes(18)]),
        ];

        FindExpiredBookings::dispatch();

        $this->assertDatabaseHas('bookings', ['id' => $bookings[0]->id, 'state' => BookingPending::$name]);
        $this->assertDatabaseHas('bookings', ['id' => $bookings[1]->id, 'state' => BookingPending::$name]);
        $this->assertDatabaseHas('bookings', ['id' => $bookings[2]->id, 'state' => BookingPending::$name]);
        $this->assertDatabaseHas('bookings', ['id' => $bookings[3]->id, 'state' => BookingExpired::$name]);
        $this->assertDatabaseHas('bookings', ['id' => $bookings[4]->id, 'state' => BookingPaid::$name]);
    }
